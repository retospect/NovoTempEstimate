...
class PeptideEncoder:
    ...
    def __init__(self, include_modifications: bool = True, max_custom_mods: int = 50):
        ...
        self.common_modifications = {
            # Phosphorylation
            'pS': ModifiedResidue('S', 'phospho', ModificationType.PHOSPHORYLATION, 79.966331),
            'pT': ModifiedResidue('T', 'phospho', ModificationType.PHOSPHORYLATION, 79.966331),
            'pY': ModifiedResidue('Y', 'phospho', ModificationType.PHOSPHORYLATION, 79.966331),
            
            # Methylation
            'mK': ModifiedResidue('K', 'methyl', ModificationType.METHYLATION, 14.015650),
            'mR': ModifiedResidue('R', 'methyl', ModificationType.METHYLATION, 14.015650),
            'me2K': ModifiedResidue('K', 'dimethyl', ModificationType.METHYLATION, 28.031300),
            'me3K': ModifiedResidue('K', 'trimethyl', ModificationType.METHYLATION, 42.046950),
            
            # Acetylation
            'acK': ModifiedResidue('K', 'acetyl', ModificationType.ACETYLATION, 42.010565),
            
            # Oxidation
            'oxM': ModifiedResidue('M', 'oxidation', ModificationType.OXIDATION, 15.994915),
            'oxW': ModifiedResidue('W', 'oxidation', ModificationType.OXIDATION, 15.994915),
            
            # Deamidation
            'dN': ModifiedResidue('N', 'deamidation', ModificationType.DEAMIDATION, 0.984016),
            'dQ': ModifiedResidue('Q', 'deamidation', ModificationType.DEAMIDATION, 0.984016),
            
            # Citrullination
            'citR': ModifiedResidue('R', 'citrullination', ModificationType.CITRULLINATION, 0.984016),
        }
    ...
    def parse_sequence(self, sequence: str) -> List[str]:
        ...
        while i < len(sequence):
            if self.include_modifications and i < len(sequence) - 1 and sequence[i:i+2] in self.common_modifications:
                # Handle short notation modifications (e.g., pS, mK)
                residues.append(str(self.common_modifications[sequence[i:i+2]]))
                i += 2
            elif self.include_modifications and i < len(sequence) - 2 and sequence[i:i+3] in self.common_modifications:
                # Handle 3-character modifications (e.g., me2K, me3K)
                residues.append(str(self.common_modifications[sequence[i:i+3]]))
                i += 3
            elif self.include_modifications and i < len(sequence) - 3 and sequence[i:i+4] in self.common_modifications:
    ...
    def encode_batch(self, sequences: List[str], pad_to_length: Optional[int] = None) -> torch.Tensor:
        """
            ...
            if '[' in residue and ']' in residue:
                ...
                for mod_residue in self.common_modifications.values():
                    if str(mod_residue) == residue:
                        info['modification_types'].add(mod_residue.modification_type.value)
                        break
                else:
    ...
    def load_vocabulary(cls, filepath: str) -> 'PeptideEncoder':
        ...
        encoder.reverse_vocabulary = {int(k): v for k, v in data['reverse_vocabulary'].items()}
...
def encode_peptide_batch(sequences: List[str], include_modifications: bool = True) -> torch.Tensor:
