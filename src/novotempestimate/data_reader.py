    ...
    def __post_init__(self):
        ...
        if self.organism is None and self.description:
            ...
            if org_match:
                ...
                organism_clean = re.sub(
                    r"\s+temp[:\s]*[0-9.]+.*$", "", organism_part, flags=re.IGNORECASE
                )
...
class TemStaProReader:
    ...
    def __init__(self, data_dir: Union[str, Path] = None):
        ...
        """
        self.data_dir = (
            Path(data_dir) if data_dir else Path(__file__).parent.parent.parent / "data"
        )
        self.temp_dir = None
    ...
    def extract_archive(
        self, archive_path: Path, extract_to: Path = None
    ) -> List[Path]:
        """
    ...
    def read_dataset(self, file_pattern: str = None) -> Iterator[ProteinRecord]:
        ...
        if file_pattern:
            ...
            available_files = [
                f for f in available_files if fnmatch.fnmatch(f.name, file_pattern)
            ]
    ...
    def filter_by_length(
        self, records: List[ProteinRecord], min_length: int = 10, max_length: int = 1000
    ) -> List[ProteinRecord]:
        """
