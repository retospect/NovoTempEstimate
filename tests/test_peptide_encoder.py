...
class TestPeptideEncoder:
    ...
    def test_encode_standard_sequence(self):
        ...
        assert torch.allclose(encoded.sum(dim=1), torch.ones(7))  # Each position sums to 1
    ...
    def test_encode_custom_modification(self):
        ...
        assert "PEP[custom_mod]TIDE" in encoder.custom_modifications or encoded[2].sum() == 1.0
    ...
    def test_malformed_bracket_notation(self):
        ...
        expected = ['P', 'E', 'P', 'X', 'i', 'n', 'c', 'o', 'm', 'p', 'l', 'e', 't', 'e']
        # Note: lowercase letters become 'X'
        expected_cleaned = ['P', 'E', 'P', 'X', 'X', 'X', 'X', 'X', 'X', 'X', 'X', 'X', 'X', 'X']
