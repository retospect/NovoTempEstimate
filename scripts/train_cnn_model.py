...
def parse_arguments():
    ...
    behavior_group.add_argument(
        "--no-save-best", action="store_true", help="Don't save best model"
    )
    behavior_group.add_argument(
        "--no-checkpoints", action="store_true", help="Don't save checkpoints"
    )
    behavior_group.add_argument(
        "--checkpoint-freq", type=int, default=5, help="Checkpoint saving frequency"
    )
    ...
    opt_group.add_argument(
        "--scheduler-factor", type=float, default=0.5, help="Scheduler reduction factor"
    )
    ...
    misc_group.add_argument(
        "--config", type=str, default=None, help="Load configuration from JSON file"
    )
    misc_group.add_argument(
        "--save-config", type=str, default=None, help="Save configuration to JSON file"
    )
    misc_group.add_argument(
        "--device",
        type=str,
...
def main():
    ...
    try:
        ...
        train_loader, val_loader, test_loader = trainer.load_data(
            data_pattern=args.data_pattern, sample_size=args.sample_size
        )
