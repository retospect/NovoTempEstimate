...
def parse_arguments():
    ...
    parser.add_argument(
        "--sample-size", type=int, default=None, help="Sample size for testing"
    )
    parser.add_argument(
        "--save-predictions", action="store_true", help="Save predictions to CSV file"
    )
    parser.add_argument(
        "--create-plots", action="store_true", help="Create evaluation plots"
    )
...
class ModelEvaluator:
    ...
    def load_model(self):
        ...
        self.model = ProteinTemperatureLSTM(vocab_size=vocab_size, device=self.device)
    ...
    def evaluate(
        ...
        print(
            f"Target range: {metrics['min_target']:.1f}°C - {metrics['max_target']:.1f}°C"
        )
        print(
            f"Prediction range: {metrics['min_prediction']:.1f}°C - {metrics['max_prediction']:.1f}°C"
        )
        print("=" * 50)
